...
{	
	...
	//长度不对，退出
	if(len <= 7){RPRMUM_pr("Usc data length err.\r\n");return FALSE;}
    //检查头是否正确
    if(data[0] != COMM_HEAD_1){RPRMUM_pr("Usc head err.\r\n");return FALSE;}
    //检查尾是否正确
	if(data[len-1] != COMM_END){RPRMUM_pr("Usc data err.\r\n");return FALSE;}
    //检查校验和
    if(data[len-2] !=RPRMUM_SumCheck(&data[1],len-3)){RPRMUM_pr("Usc checksum err.\r\n");return FALSE;}
	//数据正常，可以进行执行	
    if(data[1]==0x01){RPRMUM_ExecAnswer(data,len); return TRUE;}//响应帧
    else if(data[1]==0x02){RPRMUM_ExecInform(data,len); return TRUE;}//通知帧
    else {RPRMUM_pr("data comm tyep err.\r\n");return FALSE;}
